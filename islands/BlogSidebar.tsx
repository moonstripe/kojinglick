...
export default function Side({ markdown }: SideProps) {
    ...
    return (
        <nav class={tw`mb-4 fixed md:left-32 xl:left-32 2xl:left-1/4 top-1/4 text-base xs:invisible sm:invisible md:invisible lg:visible xl:visible dark:text-green-600`} style={{ maxWidth: "20ch" }}>
            <ul>
                {
                    titles.map((e, i) =>
                        e.slice(0, 3) === "###" ? (
